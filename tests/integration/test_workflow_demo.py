...
def test_workflow_demo_embedded_mode(tmp_path: Path) -> None:
    report = _run_demo(
        mode="embedded",
    ...
    assert sum(report["assignment_counts"].values()) == 10


def test_workflow_demo_service_mode(tmp_path: Path) -> None:
    port = _free_port()
