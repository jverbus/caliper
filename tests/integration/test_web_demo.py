...
def _run_demo(*, mode: str, db_url: str, api_url: str, api_token: str | None) -> dict[str, Any]:
    from examples.web_demo.demo import run_demo
...
def test_web_demo_embedded_mode(tmp_path: Path) -> None:
    report = _run_demo(
        mode="embedded",
        db_url=f"sqlite:///{tmp_path}/embedded-demo.db",
        api_url="http://127.0.0.1:9999",
        api_token=None,
    )
    assert report["job_id"]
    assert "## Leaders" in report["markdown"]
    assert "country=" in report["markdown"]
    assert "device=" in report["markdown"]
    assert sum(report["assignment_counts"].values()) == 12


def test_web_demo_service_mode(tmp_path: Path) -> None:
    port = _free_port()
    db_path = tmp_path / "service-demo.db"
    env = os.environ.copy()
    ...
    try:
        ...
        report = _run_demo(
            mode="service",
            db_url=f"sqlite:///{tmp_path}/ignored.db",
            api_url=f"http://127.0.0.1:{port}",
            api_token=None,
        )
        assert report["job_id"]
        assert "## Segment findings" in report["markdown"]
        assert "country=" in report["markdown"]
        assert sum(report["assignment_counts"].values()) == 12
    finally:
        process.terminate()
